...
def main() -> int:
    ap = argparse.ArgumentParser(description="Run small online ns-3 queue-driven controller sanity check.")
    ...
    ap.add_argument("--primary-rate-mbps", type=float, default=4.0)
    ap.add_argument("--background-rate-mbps", type=float, default=8.0)
    args = ap.parse_args()

    methods = [m.strip() for m in args.methods.split(",") if m.strip()]
    tag = (
        f"ns3_queue_signal_sanity_rate{args.link_rate_mbps:g}_q{args.queue_packets}"
        f"_sample{args.sample_ms:g}_pri{args.primary_rate_mbps:g}_bg{args.background_rate_mbps:g}"
    ).replace(".", "p")
    out_dir = REPO_ROOT / f"results/issue-{args.issue}/artifacts/{tag}"
    out_dir.mkdir(parents=True, exist_ok=True)

    binary_path = Path(run(["bash", "scripts/build_ns3_queue_signal.sh"]).splitlines()[-1])
        ...
        rows.append(
            {
                ...
                "primary_rate_mbps": args.primary_rate_mbps,
                "background_rate_mbps": args.background_rate_mbps,
                "primary_delivery_ratio": payload["primary_delivery_ratio"],
