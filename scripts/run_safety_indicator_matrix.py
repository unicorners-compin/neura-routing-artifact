...
def run_case(task: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "post_startup_min_reachability_ratio": min(float(row["pair_reachability_ratio"]) for row in rows),
        "post_startup_max_loop_ratio": max(float(row["loop_ratio"]) for row in rows),
        "post_startup_max_blackhole_ratio": max(float(row["blackhole_ratio"]) for row in rows),
        "final_pair_reachability_ratio": float(result["final_metrics"]["pair_reachability_ratio"]),
        "final_node_complete_route_ratio": float(result["final_metrics"]["node_complete_route_ratio"]),
    }


def main() -> int:
    ap = argparse.ArgumentParser(description="Run representative safety-indicator summaries for the NEURA paper.")
    ...
    if args.jobs <= 1:
        for idx, task in enumerate(tasks, start=1):
            ...
            print(f"[safety] {idx}/{len(tasks)} scenario={task['scenario']} method={task['method']} seed={task['seed']}")
    else:
            ...
            summary_rows.append(
                {
                    "scenario": scenario,
                    "method": method,
                    "startup_full_reachability_ms_mean": mean(
                        [float(row["startup_full_reachability_tick"]) * TICK_MS for row in rows]
                    ),
                    "startup_full_reachability_ms_ci95": ci95(
                        [float(row["startup_full_reachability_tick"]) * TICK_MS for row in rows]
                    ),
                    "post_startup_min_reachability_mean": mean(
                        [float(row["post_startup_min_reachability_ratio"]) for row in rows]
                    ),
                    "post_startup_min_reachability_worst": min(
                        float(row["post_startup_min_reachability_ratio"]) for row in rows
                    ),
                    "post_startup_max_loop_ratio_mean": mean(
                        [float(row["post_startup_max_loop_ratio"]) for row in rows]
                    ),
                    "post_startup_max_loop_ratio_worst": max(
                        float(row["post_startup_max_loop_ratio"]) for row in rows
                    ),
                    "post_startup_max_blackhole_ratio_mean": mean(
                        [float(row["post_startup_max_blackhole_ratio"]) for row in rows]
                    ),
                    "post_startup_max_blackhole_ratio_worst": max(
                        float(row["post_startup_max_blackhole_ratio"]) for row in rows
                    ),
                    "final_pair_reachability_mean": mean(
                        [float(row["final_pair_reachability_ratio"]) for row in rows]
                    ),
                    "final_node_complete_route_ratio_mean": mean(
                        [float(row["final_node_complete_route_ratio"]) for row in rows]
                    ),
                }
            )

    stem = (
        f"safety_indicator_matrix_er_n{args.nodes}_s{len(args.seeds)}"
        if args.topology == "er"
        else f"safety_indicator_matrix_{args.topology}_n{args.nodes}_s{len(args.seeds)}"
    )
