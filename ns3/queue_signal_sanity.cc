...
double
QueueNorm(const Ptr<PointToPointNetDevice>& dev, uint32_t queuePackets)
{
...
GetStaticRouting(Ptr<Node> node)
{
...
void
ReplaceSourceRoute(TopologyState* state, const std::string& newPath)
{
...
void
RecordSample(TopologyState* state, double qA, double qB, bool burstActive)
{
...
bool
BurstActive(double now)
{
    return (now >= 2.0 && now < 5.0) || (now >= 7.0 && now < 10.0);
}

void
MaybeSwitch(TopologyState* state, const std::string& nextPath, double now)
{
    ...
    {
        state->controller.nextAllowedSwitch =
            now + state->config.minSwitchMs / 1000.0;
    }
}

void
ControllerStep(TopologyState* state)
{
    const double now = Simulator::Now().GetSeconds();
    ...
    if (state->config.method == "lift")
    {
        ...
        ctl.accum = std::max(0.0, ctl.accum * (1.0 - state->config.liftAccumDecay) + state->config.liftAccumGain * diff);
        const double effCurrent = currentBase + currentQ + 0.45 * currentMemory;
        const double effAlt = altBase + altQ + 0.45 * altMemory;
        if (ctl.accum >= state->config.liftThreshold &&
            effAlt + state->config.liftGuard < effCurrent)
        {
    ...
    else if (state->config.method == "triggered_te")
    {
        ...
        const double margin = BurstActive(now) ? state->config.triggerThreshold : state->config.hysteresis;
...
void
WriteSummary(TopologyState& state)
{
...
MainBody(int argc, char* argv[])
{
    ...
    auto installLink =
        [&](Ptr<Node> a, Ptr<Node> b, const char* subnet) {
    ...
    const auto l12 = installLink(state.source, state.relayA, "10.0.1.0");
    const auto l13 = installLink(state.source, state.relayB, "10.0.2.0");
    const auto l24 = installLink(state.relayA, state.dest, "10.0.3.0");
    const auto l34 = installLink(state.relayB, state.dest, "10.0.4.0");
    const auto l25 = installLink(state.relayA, state.bgSink, "10.0.5.0");
    ...
    relayARouting->AddHostRouteTo(l12.second.GetAddress(0),
                                  l12.second.GetAddress(0),
                                  state.relayA->GetObject<Ipv4>()->GetInterfaceForDevice(l12.first.Get(1)));
    ...
    relayBRouting->AddHostRouteTo(l13.second.GetAddress(0),
                                  l13.second.GetAddress(0),
                                  state.relayB->GetObject<Ipv4>()->GetInterfaceForDevice(l13.first.Get(1)));
    ...
    PacketSinkHelper primarySinkHelper("ns3::UdpSocketFactory",
    ...
    OnOffHelper primaryOnOff("ns3::UdpSocketFactory",
                             InetSocketAddress(state.destAddress, state.primaryPort));
    primaryOnOff.SetConstantRate(DataRate(MbpsString(config.primaryRateMbps)), 1200);
    primaryOnOff.SetAttribute("StartTime", TimeValue(Seconds(0.5)));
    primaryOnOff.SetAttribute("StopTime", TimeValue(Seconds(config.simSeconds)));
    primaryOnOff.Install(state.source);

    auto addBurst = [&](double start, double stop) {
        OnOffHelper bgOnOff("ns3::UdpSocketFactory",
                            InetSocketAddress(state.bgSinkAddress, state.bgPort));
    ...
    Simulator::Schedule(Seconds(0.2), &ControllerStep, &state);
